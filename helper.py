...
def Update_yolobotter():
    ...
    if config_online_version != config_current_version:
        ...
        replace_config = True
    if config_online_version == config_current_version:
        print('Dont change Config Directly')
    ...
    download_file('https://github.com/NANOBYTECOMPUTERS/yolobotter/archive/refs/heads/main.zip', 'main.zip')
    print('Unpacking...')
    ...
    temp_aimbot_files = [
        ...
        './logic/capture.py', './logic/config_watcher.py', './logic/game.yaml', './logic/ghub_mouse.dll', './logic/buttons.py', './logic/mouse.py', './logic/visual.py', './logic/frame_parser.py', './logic/hotkeys_watcher.py',
        './media/tests/test_det.mp4', './models/model1.pt' ]

    for temp_file in temp_aimbot_files:
        try:
            if temp_file == './yolobotter-main/config.ini' and replace_config == False:
                pass
            else:
                shutil.move('yolobotter-main/{0}'.format(temp_file), temp_file)
        except:
            pass
    try:
        delete_files_in_folder('./yolobotter-main')
        os.rmdir('./yolobotter-main')
